...
KWorksheetCursor WorksheetEntry::search(const QString& pattern, unsigned flags,
...
bool WorksheetEntry::replace(const QString& replacement)
{
    Q_UNUSED(replacement);
    return false;
}

void WorksheetEntry::replaceAll(const QString& pattern, const QString& replacement, unsigned flags, KTextEditor::SearchOptions options)
{
    Q_UNUSED(pattern);
    Q_UNUSED(replacement);
    Q_UNUSED(flags);
...
QGraphicsObject* WorksheetEntry::mainTextItem() const
{
    return nullptr;
}

WorksheetTextEditorItem* WorksheetEntry::highlightItem()
{
    return nullptr;
}
